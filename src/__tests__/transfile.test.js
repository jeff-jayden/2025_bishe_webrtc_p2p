...
const mockRTCPeerConnection = vi.fn(() => ({
    ...
    getStats: vi.fn(() => Promise.resolve({
        forEach: vi.fn((callback) => {
            callback({ type: 'candidate-pair', state: 'succeeded', bytesSent: 1000, bytesReceived: 2000 });
        }),
    })),
    onicecandidate: null,
    ondatachannel: null,
...
const mockFileReader = vi.fn(() => ({
    addEventListener: vi.fn((event, callback) => {
        if (event === 'load') {
            ...
            setTimeout(() => callback({ target: { result: new ArrayBuffer(100) } }), 10);
        }
    }),
...
describe('Transfile', () => {
    ...
    it('should add files to localFilesList on drop', async () => {
        const wrapper = shallowMount(Transfile, {
            props: {
                receivedFileList: [],
                sendChannel: null,
    ...
    it('should initiate file transfer when sendData is called and channel is open', async () => {
        ...
        const mockFile = new File([new ArrayBuffer(500)], 'test.bin', { type: 'application/octet-stream' });
    ...
    it('should handle received chunk info and data', async () => {
        const wrapper = shallowMount(Transfile, {
            props: {
                receivedFileList: [],
                sendChannel: null,
        ...
        const fileInfo = { name: 'received.bin', size: 200, type: 'application/octet-stream' };
        ...
        await wrapper.vm.onReceiveChannelMessageCallback({ data: chunkInfo });
        ...
        await wrapper.vm.onReceiveChannelMessageCallback({ data: dataChunk });
        ...
        await wrapper.vm.onReceiveChannelMessageCallback({ data: firstChunkInfo });

        // Simulate receiving first data chunk
        const firstDataChunk = new ArrayBuffer(100);
        await wrapper.vm.onReceiveChannelMessageCallback({ data: firstDataChunk });
        ...
        await wrapper.vm.onReceiveChannelMessageCallback({ data: secondChunkInfo });

        // Simulate receiving second data chunk
        const secondDataChunk = new ArrayBuffer(100);
        await wrapper.vm.onReceiveChannelMessageCallback({ data: secondDataChunk });
        ...
        expect(wrapper.vm.receivedFileList.find(file => file.name === transferId).receivedFile).toBeInstanceOf(File);
    });

    // Test Case 5: handleDeleteFile removes file from localFilesList
    it('should remove file from localFilesList on handleDeleteFile', () => {
        const wrapper = shallowMount(Transfile, {
            props: {
                receivedFileList: [],
                sendChannel: null,
