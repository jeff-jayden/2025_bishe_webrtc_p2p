...
export class PeerDataChannel {
  ...
  private setupPeerConnection(): void {
    ...
    this.pc.onicecandidate = (e) =>
      e.candidate && this.onICECandidate(e.candidate);
  ...
  private handleConnectionStateChange(): void {
    if (
      ['closed', 'disconnected', 'failed'].includes(this.pc.connectionState)
    ) {
      this.dispose();
      this.onDispose();
        ...
        for (let i = 0; i < 32; i++) {
          if (offset < count) {
            const chunk = data.slice(
              this.blockSize * offset,
              this.blockSize * (offset + 1)
            );
    ...
    return this.pc
      ...
      .then(() =>
        this.pc.localDescription
          ? this.onSDP(this.pc.localDescription)
          : undefined
      )
      .catch((e) => {
        console.error(e);
        this.onError(
          e instanceof Error
            ? e
            : new Error('Unknown error during renegotiation')
        );
        this.dispose();
      });
  }
  ...
  public async setRemoteSDP(sdp: RTCSessionDescriptionInit): Promise<void> {
    try {
      ...
      console.error(e);
      this.onError(
        e instanceof Error ? e : new Error('Error setting remote SDP')
      );
      this.dispose();
    }
  }
  ...
  public getReceivedBufferSize(): number {
    return this.receiveData.chunks.reduce(
      (size, dat) =>
        size + (typeof dat === 'string' ? dat.length : dat.byteLength),
      0
    );
  }
